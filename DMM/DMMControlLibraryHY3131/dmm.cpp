...
void DMM::buttonPressed(int pPressed) {
	...
	case 5:
		...
		Flag.buzzerFlag = 1;
		Flag.vFlag = Flag.iFlag = Flag.r2wFlag = Flag.dcFlag = Flag.acFlag
				= Flag.diodeFlag = Flag.autoFlag = 0;
	...
	case 6:
		...
		Flag.diodeFlag = 1;
		Flag.vFlag = Flag.iFlag = Flag.r2wFlag = Flag.dcFlag = Flag.acFlag
				= Flag.continuityFlag = Flag.autoFlag = 0;
		Flag.buzzerFlag = 0;
		//setColor(1);
		buttonPressed(14);
...
void DMM::AutoRange() {
	...
	//current auto range
		...
		if (display.retval >= 999999999)
			...
			if (display.retval >= 0) {
				if (((display.retval >= 0.51) && nCurrentCur == 0)
						|| ((display.retval >= 5.1) && nCurrentCur == 1)
						|| ((display.retval >= 51.0) && nCurrentCur == 2)
						|| ((display.retval >= 510.0) && nCurrentCur == 3))
					buttonPressed(12);
				else if (((display.retval < 510.0) && nCurrentCur == 4)
						|| ((display.retval < 51.0) && nCurrentCur == 3)
						|| ((display.retval < 5.1) && nCurrentCur == 2)
						|| ((display.retval < 0.51) && nCurrentCur == 1))
